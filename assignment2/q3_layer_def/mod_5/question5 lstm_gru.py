...
## Answer: We see that for GRU, all iterations except the first one takes about 20s, while for LSTM, it takes about 21s.
# So on this model, LSTM performs slightly slower than GRU, we believe the reason for this to happen is because GRU only has two gates: reset and update.
# So that GRU has a less complex operation process than LSTM. As long as the dataset is not extremely large, GRU would take less time.
# However, if the dataset grows larger, GRU would be less favored because of memory exposure.
