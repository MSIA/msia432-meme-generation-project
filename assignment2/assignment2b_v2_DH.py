...
class assign2:
    ...
    def build_one_layer_lstm(self, maxlen=40, hidden_units=64):
    ...
    def build_cnn_v1(self, maxlen=40):
        ...
        model.add(Conv2D(256, kernel_size=(7, 1), activation='relu'))
        model.add(BatchNormalization())
        model.add(Conv2D(512, kernel_size=(7, 1), activation='relu'))
        model.add(BatchNormalization())
        model.add(Flatten())
        model.add(Dense(len(self.chars)))
        model.add(Activation('softmax'))
    ...
    def main(self, num_iter: int=25, freq: int=5, modeltype: str='lstm',
...
obj = assign2()
obj.model = obj.build_one_layer_lstm()
obj.main()
obj.clear()
